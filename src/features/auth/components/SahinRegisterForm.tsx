...
import { ExpertiseTagInput } from './ExpertiseTagInput';
import { formatPhoneNumber, validatePasswordRule } from '../../../shared/validation';
import type { KeyboardEvent } from 'react';

const { Title } = Typography;

interface SahinRegisterFormProps {
  form: FormInstance;
  activeView: AuthView;
  ...
  onExpertiseQueryChange: (value: string) => void;
  onAddTag: (tag: string) => void;
  onRemoveTag: (tag: string) => void;
  onExpertiseKeyDown: (event: KeyboardEvent<HTMLInputElement>) => void;
}

export function SahinRegisterForm({
  form,
  activeView,
  onViewChange,
  onFinish,
  ...
  onExpertiseQueryChange,
  onAddTag,
  onRemoveTag,
  onExpertiseKeyDown,
}: SahinRegisterFormProps) {
  const navigate = useNavigate();
  const districts = districtsForCity(watchedCity);

  useEffect(() => {
    form.setFieldsValue({ expertiseTags });
  }, [expertiseTags, form]);

  return (
    <Form form={form} className="animated-auth__ant-form" layout="vertical" onFinish={onFinish} initialValues={{ expertiseTags: [] }}>
      <Radio.Group
        value={activeView}
      ...
      <Title level={2} className="animated-auth__title">Şahin Motor Kaydı</Title>

      <div className="animated-auth__register-scroll">
        <div className="animated-auth__form-grid">
          <Form.Item name="firstName" rules={[{ required: true, message: 'Adınızı girin' }]}>
            <Input prefix={<UserOutlined />} placeholder="Ad" size="large" />
          </Form.Item>
          <Form.Item name="lastName" rules={[{ required: true, message: 'Soyadınızı girin' }]}>
        ...
        <ExpertiseTagInput
          ...
          onQueryChange={onExpertiseQueryChange}
          onAddTag={onAddTag}
          onRemoveTag={onRemoveTag}
          onKeyDown={onExpertiseKeyDown}
        />
        <Form.Item name="expertiseTags" style={{ display: 'none' }}>
          <Input />
        </Form.Item>
      ...
      <Form.Item
        ...
        rules={[{ validator: (_, value) => (value ? Promise.resolve() : Promise.reject(new Error('Kullanım koşullarını kabul etmelisiniz'))) }]}
      >
        <Checkbox>
          <Button type="link" danger className="animated-auth__link-btn" onClick={(e) => { e.stopPropagation(); onOpenKvkk(); }}>
            Kullanım Koşulları ve KVKK Metni
          </Button>
      ...
      <Button type="link" className="animated-auth__form-switch" onClick={() => navigate('/sahin/login')}>
